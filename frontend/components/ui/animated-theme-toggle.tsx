...
export const AnimatedThemeToggle = ({ className }: { className?: string }) => {
  ...
  // Avoid hydration mismatch: fall back to "light" until mounted.
...
const SolarSwitch = ({ isDark }: { isDark: boolean }) => {
  ...
  const sun = {
    checked: { opacity: 0, scale: 0, pathLength: 0 },
    unchecked: { opacity: 1, scale: 1, pathLength: 1 },
  } as const;

  const moon = {
    checked: { opacity: 1, scale: 1, pathLength: 1 },
    unchecked: { opacity: 0, scale: 0, pathLength: 0 },
  } as const;

  const transition = { duration, ease: [0.25, 0.4, 0.25, 1] } as const;
